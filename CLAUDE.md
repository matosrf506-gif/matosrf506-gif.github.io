...
本站属于**引流线**：公开 VPN 博客，目标是让真人直接点击访问 nasavpn.com 完成转化。

| 项目 | 引流线（本站）| 权重线（不在此）|
|------|-------------|--------------|
| 内容 | 公开 VPN / 独享 IP / 跨境账号话题 | 完全不碰 VPN 的中立工具内容 |
| 目标 | 真人直接点击下载 | 境内外收录 + DA |
| 流量来源 | Google 境外 + 社群 + 联盟 | 百度 / 必应 / Google 全量 |

**不要期待百度/必应中国/夸克收录**——VPN 内容在中文搜索引擎直接被过滤。  
**不要期待外链给 nasavpn.com 传 SEO 权重**——引流站的价值是直接转化，不是权重传递。

### 全隔离要求

- **不与任何其他 VPN 站互链**（包括 tongbaovpn、tonbovpn 主域、xlinkwork-glitch 站）
- 独立统计 ID（不共用 GA / 百度统计 / 任何追踪代码）
- 独立注册信息（GitHub 账号、邮箱全部独立）

### 跨站文章防重复（强制）

本站文章**禁止**与 tongbaovpn / tonbovpn 任何现有文章相同或高度相似：
...
git remote -v
# 正确输出应包含 github-matosrf506，而不是 github.com：
# origin  git@github-matosrf506:matosrf506-gif/matosrf506-gif.github.io.git (fetch)
# origin  git@github-matosrf506:matosrf506-gif/matosrf506-gif.github.io.git (push)

# 若 remote 显示 github.com，立即修正（否则会走 Speedlinc 的默认 key）：
git remote set-url origin git@github-matosrf506:matosrf506-gif/matosrf506-gif.github.io.git
...
> ⚠️ **教训（2026-06）**：GitHub 给出的初始化命令默认用 `git@github.com:...`，这会走 Speedlinc 的默认 SSH key，导致推送被拒绝（permission denied）或身份泄露。每次建仓库或克隆后，必须手动把 remote 改为 `github-matosrf506` alias 再操作。
